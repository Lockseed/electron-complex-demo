...
function merge(this: any, ...args: any[]) {
  ...
  for (let i = 0, l = args.length; i < l; i++) {
    args[i] && forEach(args[i], assignValue);
  }
  return result;
}
...
const toFlatObject = (sourceObj, destObj, filter, propFilter) => {
  ...
  do {
    ...
    sourceObj &&
    (!filter || (typeof filter === 'function' && filter(sourceObj, destObj))) &&
...
export {
  ...
  isHTMLForm,
  hasOwnProperty,
  hasOwnProperty as hasOwnProp, // an alias to avoid ESLint no-prototype-builtins detection
  reduceDescriptors,
  freezeMethods,
  toObjectSet,
  toCamelCase,
