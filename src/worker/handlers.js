    ...
    Object.entries(handlerObj).forEach(([handlerName, handler]) => {
      const handlerWithLog = async (...args) => {
        try {
          ...
          logger.debug(`[handlerWithLog] Execute handler: ${namespace}::${handlerName} in ${(end - start).toFixed(2)}ms`);

          return result;
        } catch (error) {
          logger.error(`[handlerWithLog] Execution error in worker handler: ${namespace}::${handlerName}`, error);
        }
      }
