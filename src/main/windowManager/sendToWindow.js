 ...
 */
export function broadcastToWindowContentViews(win, sendArgs = [], options = {}) {
  if (!win || !(win instanceof BrowserWindow)) {
    throw new TypeError("[broadcastToWindowContentViews] Invalid 'win'. Expected BrowserWindow, got " + typeof win);
  }
  ...
  if (!channel || typeof channel !== "string") {
    throw new TypeError("[broadcastToWindowContentViews] Invalid 'channel'. Expected non-empty string, got " + typeof channel);
  }
  ...
  win.contentView?.children?.forEach((view) => {
    if (
      ...
      helpers.waitTileCanExecuteJavascript(view.webContents).then(() => {
        logger.debug(`[broadcastToWindowContentViews] Broadcasting IPC message to contentView(id:${view.webContents.id}) with channel: ${channel}`);
...
export function sendToWindow(win, sendArgs = [], options = { broadcastToContentview: false }) {
  ...
  // channel 不允许为空
  if (!channel || typeof channel !== "string") {
    throw new TypeError("[sendToWindow] Invalid 'channel'. Expected non-empty string, got " + typeof channel);
  }
  ...
  helpers.waitTileCanExecuteJavascript(win.webContents).then(() => {
    logger.debug(`[sendToWindow] Sending IPC message to window(id:${win.id}) with channel: ${channel}`);
