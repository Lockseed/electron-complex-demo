...
import type { PromiseWithResolvers, RpcServerWorkerToMain } from '@/common/types.js';
...
const resolvers =
  Promise.withResolvers<Electron.UtilityProcess>() as PromiseWithResolvers<Electron.UtilityProcess>;
export const whenWorkerReady = resolvers.promise;
let processing = false;

export let rpcMainSide: RpcServerWorkerToMain | undefined;

/**
 * 创建 worker process 处理 IO/CPU 占用较高的任务
 * @returns {Promise<Electron.UtilityProcess>}
 */
export function setupWorkerProcess(): Promise<Electron.UtilityProcess> {
  if (processing) {
 ...
 */
export function createRpcMainSide(worker: Electron.UtilityProcess): RpcServerWorkerToMain {
  ...
  return AsyncCall(mainProcessSideImplementation, {
    channel: {
      on(listener) {
        const f = (data: unknown) => listener(data);
        worker.on('message', f);
        return () => {
 ...
 */
export async function connectWorkerToRenderer(webContents: Electron.WebContents): Promise<void> {
  ...
  webContents.on('destroyed', () => {
    try {
      ...
      logger.error(
        `[connectToRenderer] Close worker-renderer connection when renderer destroyed with error.`,
        toLogFormat(error)
      );
    }
  });
