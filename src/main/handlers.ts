...
const calculatorHandlers = {
  add(_event: Electron.IpcMainInvokeEvent, a: number, b: number) {
    logger.debug('calculator::add called with args: ', a, b);
    return a + b;
  },
};

const allHandlers: Record<string, Record<string, Handler>> = {
  calculator: calculatorHandlers,
  remoteStore: remoteStoreHandlers as unknown as Record<string, Handler>,
};
...
export function registerAPIHandlers() {
  const handleIPCMessage = async (
    ...
    const handler = allHandlers[namespace]?.[handlerName];
