...
describe('createRemoteAPI', () => {
  ...
  it('skips invalid handler maps', () => {
    ...
    const apis = createRemoteAPI([['bad', 'not-an-array' as any]] as any, async () => {});
...
describe('createRemoteEvent', () => {
  it('creates namespaced event subscriptions from an event map', () => {
    const register = vi.fn((channel: string) => (listener: (...args: unknown[]) => void) => {
      listener(channel);
      return () => {};
    });
    const listener = vi.fn();

    const events = /** @type {any} */ createRemoteEvent(
